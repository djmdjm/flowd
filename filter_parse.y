...
static struct filter_list *filter_list;
static u_int32_t *store_mask;
static int verbose;
...
conf_main	: STORE logspec	{ *store_mask |= $2; }
		;
...
filterrule	: action tag quick match_agent match_src match_dst match_proto match_tos
		{
			...
			TAILQ_INSERT_TAIL(filter_list, r, entry);
		}
...
int
parse_filter(const char *path, FILE *f, int mverbose, 
    struct filter_list *mfilter_list, u_int32_t *mstore_mask)
{
	struct sym		*sym, *next;

	filter_list = mfilter_list;
	store_mask = mstore_mask;
	*store_mask = 0;
	verbose = mverbose;
	...
	/* Default to retaining all fields */
	if (*store_mask == 0)
...
void
dump_filter(struct filter_list *flist, u_int32_t smask,
    const char *prefix)
{
	struct filter_rule *fr;
#define DCPR(a) ((a) == NULL ? "" : a), ((a) == NULL ? "" : ": ")
	logit(LOG_DEBUG, "%s%s# store mask %08x", DCPR(prefix), smask);

	TAILQ_FOREACH(fr, flist, entry)
